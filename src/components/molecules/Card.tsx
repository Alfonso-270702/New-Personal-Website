...
const Card = ({
  ...
  return (
    <div className="flex flex-col gap-[28px]">
      ...
      <div>
        <p className="text-[36px] text-white font-medium">{title}</p>
        <p className="text-[32px] text-[#50849C]">@{place}</p>
      </div>
      {jobDescription.length > 0 ? (
        <div className="pl-[28px]">
          <ul className="list-disc text-white list-outside text-[20px]">
      ...
      {applications && (
        <div>
          ...
          <p className="text-[20px] text-white font-medium">{applications}</p>
        </div>
      )}
      {techStacks && (
        <div>
          <p className="text-[#D3D9D4] text-[18px]">TECH STACKS</p>
          <p className="text-[20px] text-white font-medium">{techStacks}</p>
        </div>
      )}
    </div>
  );
